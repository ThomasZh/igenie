 ...
 * @author thomas
 */
public class SupDeviceServer
{
	public static void main(String[] args)
			throws IOException
	{
		logger.info("service unit provider for device server is starting...");
		initSpringApp();

		logger.info("init device service manager.");
		DeviceCacheManager accountCacheManager = GenericSingleton.getInstance(DeviceCacheManager.class);
		accountCacheManager.init();

		startMinaServer();
		logger.info("service unit provider for device server start success!");
	}
			...
			throws IOException
	{
		...
		logger.info("service unit provider for device Server is listenig at port: " + gcv.getSupPort());
	}

	private final static Logger logger = LoggerFactory.getLogger(SupDeviceServer.class);
