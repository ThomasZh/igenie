		...
		extends IoHandlerAdapter
{
			...
			throws Exception
	{
		if (message instanceof TlvObject && message != null) {
			...
			logger.info("sessionId=[" + session.getId() + "]|commandTag=[" + tag + "]|execute command start.");

			SupReqCommand reqCmd = null;
			try {
				// decode all the message to request command
				reqCmd = (SupReqCommand) SupSessionCommandParser.decode(pkg);
			} catch (UnsupportedEncodingException ue) {
				logger.warn("sessionId=[" + session.getId() + "]|commandTag=[" + tag + "]|ErrorCode=["
						+ ErrorCode.ENCODING_FAILURE
						+ "]|This tlv pkg has no implementation in SupSessionCommandParser.decode() from "
			...
			if (respCmd != null) {
				...
				try {
					tResp = SupSessionCommandParser.encode(respCmd);
				} catch (Exception e) {
					logger.error("sessionId=[" + session.getId() + "]|commandTag=[" + tag + "]|ErrorCode=["
