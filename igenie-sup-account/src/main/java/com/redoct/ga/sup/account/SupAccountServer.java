 ...
 * @author thomas
 */
public class SupAccountServer
{
	public static void main(String[] args)
			throws IOException
	{
		logger.info("service unit provider for account server is starting...");
		initSpringApp();
		...
		startMinaServer();
		logger.info("service unit provider for account server start success!");
	}
			...
			throws IOException
	{
		...
		logger.info("service unit provider for account Server is listenig at port: " + gcv.getSupPort());
	}

	private final static Logger logger = LoggerFactory.getLogger(SupAccountServer.class);
