 ...
 * @author thomas
 */
public class SupMailServer
{
	public static void main(String[] args)
			throws IOException
	{
		logger.info("service unit provider for send mail is starting...");
		initSpringApp();

		startMinaServer();
		logger.info("service unit provider for send mail start success!");
	}
			...
			throws IOException
	{
		...
		logger.info("service unit provider for send mail is listenig at port: " + gcv.getSupPort());
	}

	private final static Logger logger = LoggerFactory.getLogger(SupMailServer.class);
