...
@Controller
public class FriendInviteViewController
{
			...
			throws IOException
	{
		...
		model.setViewName("friend-invite");
		...
		if (isValidCode && session.getAttribute("user") == null) {
			...
			try {
				...
				if (supAccountService.verifyExist(GlobalArgs.ACCOUNT_LOGIN_BY_WECHAT, unionid)) {
					...
					accountId = supAccountService.createAccount(nickname, headImgUrl, desc, currentTimestamp);
					supAccountService.createLogin(accountId, GlobalArgs.ACCOUNT_LOGIN_BY_WECHAT, unionid,
							currentTimestamp);
					logger.info("accountId=[" + accountId + "]|nickname=[" + nickname + "]| register success)");
				}
			} catch (Exception e) {
				e.printStackTrace();
			}
		...
		model.addObject("ekey", accountId);
		return model;
	}
	...
	private final static Logger logger = LoggerFactory.getLogger(FriendInviteViewController.class);
