...
public class JoinActionController
{
			...
			throws IOException
	{
		...
		if (myAccountId == null) {
			...
			if (user == null) {
				return new ModelAndView("redirect:/joinFailure");
			} else {
				Map map = new HashMap();
				map.put("ekey", user.getAccountId());
			...
			if (!talentService.isJoin(myAccountId)) {
				talentService.join(myAccountId, timestamp);
			}

			Map map = new HashMap();
			map.put("ekey", myAccountId);
			return new ModelAndView(new RedirectView("talent-profile.htm"), map);
		}
	}

	@RequestMapping(value = "/joinFailure", method = RequestMethod.GET)
	...
	private final static Logger logger = LoggerFactory.getLogger(JoinActionController.class);
