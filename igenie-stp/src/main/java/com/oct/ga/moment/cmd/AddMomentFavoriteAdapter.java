...
import com.oct.ga.comm.cmd.moment.AddFavoriteMomentReq;
...
public class AddMomentFavoriteAdapter
		extends StpReqCommand
{
	public AddMomentFavoriteAdapter()
	{
		super();
	...
	{
		reqCmd = new AddFavoriteMomentReq().decode(tlv);
			...
			throws Exception
	{
		...
		try {
			...
			momentService.addMomentFavorite(momentId, myAccountId, currentTimestamp);
			//int num = momentService.queryFavoriteNum(momentId);
			//momentService.modifyFavoriteNum(momentId, ++num);
			int num = momentService.countFavoriteNum(momentId);
			momentService.modifyFavoriteNum(momentId, num);

			String momentOwnerId = momentService.queryMomentOwner(momentId);
			momentService.addLog(momentId, myAccountId, GlobalArgs.TASK_ACTION_MOMENT_FAVORITE, null, momentOwnerId,
					currentTimestamp);
			...
			log.setActionTag(GlobalArgs.TASK_ACTION_MOMENT_FAVORITE);
			log.setToActionId(momentId);
			taskService.addLog(log, currentTimestamp);

			taskService.addLogExtend(log.getLogId(), myAccountId, channelId, GlobalArgs.TASK_ACTION_MOMENT_FAVORITE,
					GlobalArgs.SYNC_STATE_RECEIVED, currentTimestamp);
			
			IoSession session = this.getSession();
			AddFavoriteMomentResp respCmd = new AddFavoriteMomentResp(sequence, ErrorCode.SUCCESS);
			...
			if (!future.isWritten()) {
				...
				logger.info("add moment favorite success");
			}
			
			if (!myAccountId.equals(momentOwnerId)) {
				taskService.addLogExtend(log.getLogId(), momentOwnerId, channelId,
						GlobalArgs.TASK_ACTION_MOMENT_FAVORITE, GlobalArgs.SYNC_STATE_NOT_RECEIVED, currentTimestamp);
			...
			AddFavoriteMomentResp respCmd = new AddFavoriteMomentResp(sequence, ErrorCode.UNKNOWN_FAILURE);
			return respCmd;
		}
	}

	private AddFavoriteMomentReq reqCmd;

	private final static Logger logger = LoggerFactory.getLogger(AddMomentFavoriteAdapter.class);
